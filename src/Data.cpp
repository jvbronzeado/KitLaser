...
void Data::read(){
	...
	// Alocar matriz 2D
	distMatrix = std::shared_ptr<double*[]>(new double *[ dimension ]); //memoria dinâmica (matrix 2D)
...
{ // Get the name of instance
	...
	std::string::size_type loc = instaceName.find_last_of(".", instaceName.size() );
	std::string::size_type loc2 = instaceName.find_last_of("/", instaceName.size() );
